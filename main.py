...
class Display:
    def __init__(self):
        ...
        self.bg_color = (0,30,30)
        self.bg.fill(self.bg_color)
#        self.font = py.font.Font("fonts/freesansbold.ttf",15)
...
#                    self.planets_vel[-1] = arr([x,y,0])/self.zoom
    ...
    def draw_texts(self):
        surf = self.font.render("fps: "+str(int(self.clock.get_fps())),True,WHITE,self.bg_color)
        self.win.blit(surf,(0,0))
        surf = self.font.render("Zoom : "+str(self.zoom),True,WHITE,self.bg_color)
        self.win.blit(surf,(0,20))
        surf = self.font.render("Planets: "+str(len(self.planets)),True,WHITE,self.bg_color)
        self.win.blit(surf,(0,40))
        mix,miy = py.mouse.get_pos()
        x = self.origin[0] + mix/self.zoom
        y = self.origin[1] + miy/self.zoom
        surf = self.font.render(f"{int(x)},{int(y)}",True,WHITE,self.bg_color)
        ...
        if self.hovering_planet != -1:
            surf = self.font.render(f"Planet Name: {self.planets_names[self.hovering_planet]}",True,WHITE,self.bg_color)
    ...
    def construct_galaxy(self,px,vx):
        ...
        center_name = generate_name()
        if len(pos) > 0:
            self.spawn_particle(first_point,vx,first_particle_mass,center_name)
        else:
            self.spawn_particle(first_point,vx,first_particle_mass,center_name)
        ...
        for i in range(n_particles//2):
            ...
            name = generate_name()
            self.spawn_particle(p,vdir,mm,name)
